...
app.post('/api/research', async (req, res) => {
  ...
  const session: Session = {
    id, query, phase: 'analyzing', createdAt: Date.now(), subqueries, results: {}, activity: [], paused: false,
  };
  sessions.set(id, session);

  setImmediate(() => orchestrate(session).catch((e) => {
    console.error(e);
  }));
...
async function orchestrate(session: Session) {
  try {
    ...
    // Launch all subqueries with limiter
    await Promise.allSettled(
      session.subqueries.map((sq) =>
        limiter.schedule(() => runParallelTask(session, sq))
      )
    );
...
async function callParallel(mainQuery: string, subQuery: string): Promise<ParallelResult> {
  ...
  // Use Responses API with json_schema for structured outputs
  ...
  // @ts-expect-error: The SDK typings are flexible; coerce to any
  const out = content?.content?.[0]?.text || content?.content?.[0]?.input_text;
  if (typeof out === 'string') {
    return JSON.parse(out) as ParallelResult;
  }
  // If the SDK returns already-structured JSON
  // @ts-expect-error
  if (content?.content?.[0]?.type === 'output_text') {
    // @ts-expect-error
    return JSON.parse(content.content[0].text) as ParallelResult;
  }
